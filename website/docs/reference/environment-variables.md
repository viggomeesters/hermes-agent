...
| `HERMES_AGENT_FIRST_NOTIFY_DELAY` | Gateway: optional one-time first-status deadline in seconds; suppressed when visible tool/interim progress already occurred. Prefer `agent.gateway_first_notify_delay` in `config.yaml`. |
